...
def load_preps(config: str):
    logging.basicConfig(filename="cov.log", level=logging.DEBUG)
    with open(config) as file:
        config_data = yaml.load(file, Loader=yaml.FullLoader)
    lims = Lims(config_data["BASEURI"], config_data["USERNAME"], config_data["PASSWORD"])
    arnold_host = config_data["ARNOLD_HOST"]

    processes = lims.get_processes(type="Aggregate QC (Library Validation) (Cov) v1")

    for process in processes:
        try:
            all_step_documents = build_step_documents(prep_type="cov", process=process, lims=lims)
