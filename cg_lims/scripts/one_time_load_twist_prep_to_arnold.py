...
def load_preps(config: str):
    logging.basicConfig(filename="twist.log", level=logging.DEBUG)
    with open(config) as file:
        config_data = yaml.load(file, Loader=yaml.FullLoader)
    lims = Lims(config_data["BASEURI"], config_data["USERNAME"], config_data["PASSWORD"])
    arnold_host = config_data["ARNOLD_HOST"]

    processes = lims.get_processes(type="Aggregate QC (Library Validation) TWIST v2")

    for process in processes:
        try:
            all_step_documents = build_step_documents(prep_type="twist", process=process, lims=lims)
