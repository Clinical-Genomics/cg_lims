...
def load_preps(config: str):
    logging.basicConfig(filename="micro.log", level=logging.DEBUG)
    with open(config) as file:
        config_data = yaml.load(file, Loader=yaml.FullLoader)
    lims = Lims(config_data["BASEURI"], config_data["USERNAME"], config_data["PASSWORD"])
    arnold_host = config_data["ARNOLD_HOST"]

    processes = lims.get_processes(type="CG002 - Normalization of microbial samples for sequencing")

    for process in processes:
        try:
            all_step_documents = build_step_documents(prep_type="micro", process=process, lims=lims)
