...
def test_tube_barcode(lims):
    ...
    get_barcode_set_udf(
        artifacts=artifacts, artifact_udf=barcode_udf, container_type=container, measurement=False
    )
...
def test_plate_barcode(lims):
    ...
    assert f"No barcode assigned." in error_message.value.message
...
def test_no_container_type(lims):
    ...
    # WHEN running get_barcode_set_udf
    get_barcode_set_udf(
        artifacts=artifacts, artifact_udf=barcode_udf, container_type="", measurement=False
    )
...
def test_invalid_value(lims):
    ...
    assert f"Samples ACC7239A44, are missing udf container or udf " in error_message.value.message
...
def test_missing_value(lims):
    ...
    # exists.
    assert f"No barcode assigned. Check parameters." in error_message.value.message
...
def test_container_on_measurement(lims):
    ...
    # WHEN running get_barcode_set_udf
    get_barcode_set_udf(
        artifacts=measurements, artifact_udf=barcode_udf, container_type=container, measurement=True
    )
